...
export const getAllProducts = async (req, res, next) => {
  try {
    const resultPerPage = 8; // ek page me products
    ...
    console.log("Query copy (before parsing filters):", queryCopy);

    // ✅ Parse filters into MongoDB compatible format
    const filters = {};
    for (const key in queryCopy) {
      if (key.includes("[")) {
        const [field, operator] = key.split("["); // e.g., price[gte]
    ...
    console.log("Filters applied (MongoDB format):", filters);

    // ✅ Total products in DB
    const productsCount = await Product.countDocuments();
    console.log("Total products in DB:", productsCount);

    // ✅ Total products after filters
    ...
    console.log("Skip value for pagination:", skip);
    ...
    res.status(200).json({
      ...
      currentPage: page, // optional, frontend convenience
    });
  } catch (error) {
